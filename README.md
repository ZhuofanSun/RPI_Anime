...
- `./scripts/sync_to_pi.sh` now refuses to sync if either mount target is configured in `/etc/fstab` but has fallen back to `/`, which protects the SD card from accidental writes.
